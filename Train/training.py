...
def load_data(mat_file_path, width=28, height=28, max_=None, verbose=True):
    ...
    pickle.dump(mapping, open('best_model/mapping.p', 'wb' ))
...
def train(model, training_data, callback=True, batch_size=256, epochs=10):
    ...
    if callback == True:
        ...
        mcCallBack = keras.callbacks.ModelCheckpoint(filepath='best_model/model.h5', monitor='val_loss', 
    ...
    model.fit(x_train, y_train,
              ...
              verbose=1,
              validation_data=(x_test, y_test),
              callbacks=[esCallBack] if callback else None)
    ...
    model.fit(x_train, y_train,
              ...
              verbose=1,
              validation_data=(x_test, y_test),
              callbacks=[mcCallBack] if callback else None)
    ...
    with open("best_model/model.yaml", "w") as yaml_file:
        yaml_file.write(model_yaml)
    save_model(model, 'best_model/model.h5')
